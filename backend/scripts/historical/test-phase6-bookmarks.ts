...
async function setupTestData() {
  ...
  // Verify user directly in DB (Phase 10 security compliance)
  await prisma.user.update({ where: { email: testEmail }, data: { isVerified: true } });
  ...
  AUTH_USER_ID = userObj?.id || loginRes.body.user?.id;
