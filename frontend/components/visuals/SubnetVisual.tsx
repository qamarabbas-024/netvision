    ...
    <div className="p-6 rounded-2xl glass-panel border border-[#272732] flex flex-col gap-6">
      <div>
        ...
        <h3 className="text-xl font-bold text-white flex items-center gap-2">
          <Network className="w-5 h-5 text-[#00f0ff]" /> Subnet CIDR Mask Host Visualizer
        </h3>
      </div>
      ...
      <div className="p-6 rounded-xl bg-[#09090b] border border-[#272732] flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between font-mono text-xs text-zinc-400">
            <span>SELECT CIDR PREFIX: <strong className="text-[#00f0ff]">192.168.1.0 /{cidr}</strong></span>
            <span>SUBNET MASK: <strong className="text-purple-400">{details.mask}</strong></span>
          </div>
        ...
        <div className="p-4 rounded-xl bg-[#121217] border border-[#272732]">
          <h4 className="text-xs font-bold text-white mb-1 flex items-center gap-1.5">
            <Server className="w-4 h-4 text-emerald-400" /> Scenario Application
          </h4>
          <p className="text-xs text-zinc-300 leading-relaxed font-mono">{details.desc}</p>
        </div>
      </div>
    </div>
  );
